...
namespace Actual_DB_Test
{
    ...
    {
        private readonly MySqlConnection _connection = new("SERVER=localhost;DATABASE=photos_storage_server;username=root;PASSWORD=Ph0t0s_Server;");
        ...
        {
            MySqlCommand cmd = new("INSERT IGNORE INTO media VALUES (@path, @dateAdded, @dateTaken)", _connection);
        ...
        public void CreateAlbum(string name)
        {
            try
            {
                MySqlCommand cmd = new MySqlCommand("INSERT INTO albums (Name) VALUES (@name)", _connection);
        ...
        public void UpdateAlbumCover(string albumName, string path)
        {
            ...
            try
            {
                MySqlCommand cmd = new("UPDATE albums SET album_cover=@path WHERE id=@ID", _connection);
                cmd.Parameters.AddWithValue("@path", path);
        ...
        public int GetAlbumId(string name)
        {
            ...
            try
            {
                ...
                MySqlCommand selectCmd = new MySqlCommand("SELECT id FROM albums WHERE name=@name", _connection);
        ...
        public void DeleteAlbum(string name)
        {
            try
            {
                ...
                if (reader.HasRows) //Check if there is actually a row to read. If reader.Read() is called and there isn't, a nasty exception is raised.
                {
                    ...
                    MySqlCommand delCmd = new MySqlCommand("DELETE FROM album_entries WHERE album_id=@id", _connection);
        ...
        public void AddToAlbum(string path, int albumId)
        {
            try
            {
                ...
                MySqlCommand cmd = new MySqlCommand("INSERT IGNORE INTO album_entries VALUES (@path, @albumID, @date_added_to_album)", _connection);
                cmd.Parameters.AddWithValue("@path", path);
        ...
        public void RemoveFromAlbum(string path, int albumId)
        {
            try
            {
                MySqlCommand cmd = new("DELETE FROM album_entries WHERE album_id=@albumID AND path=@path", _connection);
                cmd.Parameters.AddWithValue("@albumID", albumId);
                cmd.Parameters.AddWithValue("@path", path);
                cmd.ExecuteNonQuery();
            }
        ...
        public void DeleteItem(string path)
        {
            try
            {
                ...
                MySqlCommand cmd = new("INSERT INTO media_trash SELECT * FROM media WHERE path=@path", _connection);
                cmd.Parameters.AddWithValue("@path", path);
                cmd.ExecuteNonQuery();
        ...
        public void RestoreItem(string path)
        {
            ...
            {
                //Copy item from media to trash
                MySqlCommand cmd = new("INSERT INTO media SELECT * FROM media_trash WHERE path=@path", _connection);
                cmd.Parameters.AddWithValue("@path", path);
                cmd.ExecuteNonQuery();
        ...
        public List<Media> LoadMediaTable()
        {
            ...
            try
            {
                MySqlCommand cmd = new("SELECT * FROM media ORDER BY date_taken DESC", _connection);
                cmd.ExecuteNonQuery();
        ...
        public List<Media> SelectAlbum(string name)
        {
            ...
            try
            {
                MySqlCommand cmd = new("SELECT a.path, m.date_taken, a.date_added_to_album FROM media AS m INNER JOIN album_entries AS a ON m.path=a.path WHERE album_id=@ID", _connection);
                cmd.Parameters.AddWithValue("@ID", id);
                cmd.ExecuteNonQuery();
                MySqlDataReader reader = cmd.ExecuteReader();
        ...
        public void UpdateDateTaken(string oldPath, string newPath)
        {
            try
            {
                MySqlCommand updateMediaCmd = new("UPDATE media SET path=@newPath WHERE path=@oldPath", _connection);
                ...
                MySqlCommand updateAlbumEntriesCmd = new("UPDATE album_entries SET path=@newPath WHERE path=@oldPath", _connection);
        ...
        {
            MySqlCommand cmd = new("DELETE FROM media; DELETE FROM media_trash; DELETE FROM albums; DELETE FROM album_entries; DELETE FROM album_entries_trash;", _connection);
            cmd.ExecuteNonQuery();
        }
    }
}