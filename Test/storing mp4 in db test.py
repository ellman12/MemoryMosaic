...
for dirPath, _, files in os.walk(dirToSort):
    for file in files:
        ...
        if (".mp4" in file):
            print(file)
            results = subprocess.run(["ffprobe", "-v", "0", "-print_format", "compact=print_section=0:nk=1", "-show_entries", "format_tags=creation_time", file], stdout = subprocess.PIPE)
            output = results.stdout.decode().strip()
            timestampStr = dateutil.parser.isoparse(output)
        else:
            print(file)
